...
class population():
  def __init__(self, generation, length, client_keys, depot, vehicle_range, demands, vehicle_capacity, time_windows, distances, B, initialize=False):
    ...
    self.client_number = len(client_keys)
    ...
    # print("vehicle range =>", self.vehicle_range)
    if self.vehicle_range > 1:
    ...
    # print(f"Depot {depot} Client_number {self.client_number}")
  ...
  def demand_check(self, sequence):
    ...
    for i in range(len(sequence)):
      total_demand += self.client_demands[self.client_keys[i]]
  ...
  def initialize(self):
    ...
    i = 0
    rep = 0
    while i <= self.size and rep <= 100000:
      rep = rep+1
      ...
      for subroute in routes: ## we assume the time is equal to the distance
        dist = subroute_distance(subroute, self.distances, self.client_keys, self.depot_id)
        if dist >= self.B:
          time_flag = False
          break

      #print(f"Demand flag {demand_flag} and Time flag {time_flag}, {len(routes)<= max_nv}")
      if demand_flag and time_flag and len(routes)<= max_nv: ## the chromosome is accepted
        ...
        i += 1
##
##    ## TODO: Added Temporarily to test the module
##    print(f"Initialization >> population length:{len(self.population)},size:{self.size}")
  ...
  def merging_sub_routes(self, individual, distances):
    ...
    if len(routes)>=2:
      ...
      for route_index in range(len(routes)):
        subroute_dists[route_index] = subroute_distance(routes[route_index], distances, self.client_keys, self.depot_id)
