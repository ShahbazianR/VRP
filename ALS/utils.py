...
import copy
def route_time_eval(tour, distance_matrix, C_TW, C_service):
    route = copy.deepcopy(tour)[1:-1]
    route_service_time = 0
    route_tw_violation = 0
    total_time = distance_matrix[(tour[0],route[0])] + distance_matrix[(route[-1], tour[0])]
    ...
    while ind < len(route)-1:
        ...
        if total_time > C_TW[i][1]:
            route_tw_violation += (total_time-C_TW[i][1])
        elif total_time < C_TW[i][0]:
            route_tw_violation += (C_TW[i][0]-total_time)
            total_time = C_TW[i][0]
